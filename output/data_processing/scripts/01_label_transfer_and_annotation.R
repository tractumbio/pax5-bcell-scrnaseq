...
# (bcells_raw_filtered.rds) — and assigns B-cell identity labels to each cell in
...
#
# OUTPUT
#   data/bcells_annotated.rds   — fully annotated Seurat object with FinalLab/2/3
#   output/data_processing/supporting_data/umap_coordinates_and_labels.csv — UMAP coordinates + all label columns per cell
...
reference <- readRDS("data/reference_atlas_lee.rds")
query     <- readRDS("data/bcells_raw_filtered.rds")
...
saveRDS(b.query, "data/bcells_annotated.rds")
...
# Standardise label spelling so all downstream scripts can use "PreproB"
# (the reference atlas labels it "Prepro B" with a space).
b.query@meta.data <- b.query@meta.data %>%
  mutate(across(c(FinalLab, FinalLab2, FinalLab3),
                ~gsub("Prepro B", "PreproB", as.character(.))))
...
write.csv(umap_coords, "output/data_processing/supporting_data/umap_coordinates_and_labels.csv", row.names = FALSE)

saveRDS(b.query, "data/bcells_annotated.rds")

message("Done. Annotated object saved to data/bcells_annotated.rds")
