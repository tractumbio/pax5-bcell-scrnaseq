...
#
# This script loads a labelled Seurat object (anselm_labeled.rds) and tests whether
...
write.csv(per_cell, file.path(out_dir, "pax5_per_cell_metadata_expression.csv"), row.names = FALSE)
...
print(res_sct)
write.csv(res_sct, file.path(out_dir, "pax5_DE_MUT_vs_WT_SCT.csv"), row.names = FALSE)

# --- Method 2: RNA assay (negbinom on raw counts) ---
message("\n=== RNA assay (negbinom) ===")
res_rna <- run_de(seu, cell_types, assay = "RNA", slot = "counts", test = "negbinom")
print(res_rna)
write.csv(res_rna, file.path(out_dir, "pax5_DE_MUT_vs_WT_RNA.csv"), row.names = FALSE)
...
ggsave(file.path(out_dir, "pax5_lollipop_SCT.pdf"), p_sct, width = 7, height = 4)
ggsave(file.path(out_dir, "pax5_lollipop_RNA.pdf"), p_rna, width = 7, height = 4)
